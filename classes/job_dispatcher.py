...
class JobDispatcherRESTAPI(JobDispatcher):
    ...
    def parameters(self) -> str:
        response = requests.get(
            f"https://www.ebi.ac.uk/Tools/services/rest/{self.tool_name}/parameters"
        )
        return response.text

    def parameters_details(self, parameter: str) -> str:
        response = requests.get(
            f"https://www.ebi.ac.uk/Tools/services/rest/{self.tool_name}/parameters/{parameter}"
        )
        return response.text
    ...
    def submit_job(self, parameter: str) -> str:
        ...
        response = requests.post(
            f"https://www.ebi.ac.uk/Tools/services/rest/{self.tool_name}/run",
            data=parameter,
        )
        return response.text
    ...
    def status(self, job_id: str) -> str:
        response = requests.get(
            f"https://www.ebi.ac.uk/Tools/services/rest/{self.tool_name}/status/{job_id}"
        )
        return response.text

    def result_types(self, job_id: str) -> str:
        response = requests.get(
            f"https://www.ebi.ac.uk/Tools/services/rest/{self.tool_name}/resulttypes/{job_id}"
        )
        return response.text
    ...
    def result(self, job_id: str, result_type: str) -> any:
        ...
        response = requests.get(
            f"https://www.ebi.ac.uk/Tools/services/rest/{self.tool_name}/result/{job_id}/{result_type}"
        )
